...
定义模式弱安全返回宏，`#define PATTERN_SAFE_CHECK_WEAK() ...` 在弱失败时继续执行，而不返回
定义模式匹配计算宏 `#define PATTERN_MATCH(x, y, e) ...` （在 x 是零值时，执行对 y 赋值；否则，执行比较，如果比较失败，则返回失败错误码 e. 并定义两个额外的宏用于控制是强失败，还是弱失败）
