        ...
        if (TREE_CODE(lhs) == MEM_REF || TREE_CODE(lhs) == INDIRECT_REF) {
            ...
            if (TREE_CODE(ptr) == SSA_NAME) {
                ...
                if (def_stmt && gimple_code(def_stmt) == GIMPLE_ASSIGN) {
                    ...
                    if (is_double_pointer_access(def_rhs, info)) {
...
// 插件回调：注册pass
static void register_callback(void *gcc_data, void *user_data) {
    struct register_pass_info pass_info;
    
    pass_info.pass = new double_pointer_access_pass(g);
    pass_info.reference_pass_name = "ipa-cp";
